...
{
	Creature *target = (Creature*)parent->Find(args[1], CREATURE)==NULL? (NPC*)parent->Find(args[1], NPC_TYPE): (Creature*)parent->Find(args[1], CREATURE);
...
void Player::Reward(const vector<string>& args) {
	if (args.size()==2) {
		NPC* npc = (NPC*)parent->Find(args[1], NPC_TYPE);
		npc->reward(this);
	}
